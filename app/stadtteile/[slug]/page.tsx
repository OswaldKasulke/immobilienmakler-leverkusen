...
export default async function DistrictPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="district-page">
    ...
    <section className="district-hero" style={image?{backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.72),rgba(0,0,0,.1)),url(${image.src})`,backgroundPosition:"center",backgroundSize:"cover",backgroundRepeat:"no-repeat"}:undefined}><div><p className="eyebrow light">Makler {district.name}</p><h1>Immobilienmakler {district.name}</h1><p>Immobilien verkaufen und bewerten – mit persönlicher Beratung durch den Standort Leverkusen.</p><a className="button gold" href="/immobilienbewertung/">Immobilienbewertung {district.name}</a></div>{image&&<a className="district-photo-credit" href={image.source} target="_blank" rel="noreferrer">Quelle: Wikipedia ↗</a>}</section>
    ...
    <section className="district-contact section" id="kontakt"><div><p className="eyebrow light">Kostenlose Erstberatung</p><h2>Immobilienbewertung in {district.name}</h2><p>Unverbindliche Anfrage an unser Team in Leverkusen.</p></div><div><a className="button gold" href="/immobilienbewertung/#bewertung">Bewertung anfragen</a><a href="tel:+4922049147881">+49 2204 914 7881</a></div></section>
