...
export default async function DistrictPage({params}:{params:Promise<{slug:string}>}){
  ...
  const structuredData=graphSchema([
    ...
    breadcrumbSchema([{name:"Startseite",url:siteUrl},{name:"Leverkusen",url:`${siteUrl}/leverkusen/`},{name:district.name,url}]),
  ...
  return <main className="district-page">
    ...
    <section className="district-hero" style={image?{backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.72),rgba(0,0,0,.1)),url(${image.src})`}:undefined}><div><p className="eyebrow light">Makler {district.name}</p><h1>Immobilienmakler {district.name}</h1><p>Immobilien verkaufen und bewerten – mit persönlicher Beratung durch den Standort Leverkusen.</p><a className="button gold" href="#kontakt">Immobilienbewertung {district.name}</a></div>{image&&<a className="district-photo-credit" href={image.source} target="_blank" rel="noreferrer">Foto: {image.author} · Wikimedia Commons · {image.license} ↗</a>}</section>
    <section className="district-intro section"><div><p className="eyebrow">Stadtteilprofil</p><h2>{district.name} im Porträt</h2><p className="lead">{district.profile}</p><p>Für eine Immobilienbewertung werden neben der konkreten Lage auch Grundstück, Baujahr, Zustand, Nutzung und Energieeffizienz betrachtet.</p></div><aside><span>Amtliche Zuordnung</span><strong>Bezirk {district.code}</strong><span>Stadtteil</span><strong>{district.name}</strong><small>Quelle: Stadt Leverkusen</small></aside></section>
    ...
    <section className="faq-section section"><div className="section-head"><div><p className="eyebrow">Kurz beantwortet</p><h2>Haus, Wohnung oder Grundstück in {district.name} verkaufen.</h2></div><p>Antworten für Eigentümer in Leverkusen-{district.name}.</p></div><div className="faq-grid">{districtFaq.map(item=><details className="faq-item" key={item.question}><summary>{item.question}<span>+</span></summary><div><p>{item.answer}</p></div></details>)}</div><p className="editorial-note">Stand: 25.08.2026 · Redaktion: Stark &amp; Hoffmann Immobilien · Marktdaten werden mit amtlichen Quellen belegt.</p></section>
