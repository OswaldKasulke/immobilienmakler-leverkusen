...
export default function BewertungsForm(){
  ...
  return <div className="valuation-tool"><div className="progress" aria-label={`Schritt ${step} von 6`}>{[1,2,3,4,5,6,7].map(n=><span className={n<=step?"active":""} key={n}>{n}</span>)}</div><div className="tool-panel">
    ...
    <div className="tool-actions">{step>1&&<button type="button" className="button outline" onClick={back}>{step===6?"Angaben ändern":"Zurück"}</button>}{step<5&&<button type="button" className="button gold" disabled={!canContinue} onClick={next}>Weiter</button>}{step===5&&<button type="button" className="button gold" disabled={!canContinue||sendStatus==="sending"} onClick={evaluate}>Bewertung berechnen</button>}</div>
  </div></div>;
}
