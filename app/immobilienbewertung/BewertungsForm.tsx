...
export default function BewertungsForm(){
  ...
  return <div className="valuation-tool"><div className="progress" aria-label={`Schritt ${step} von 7`}>{[1,2,3,4,5,6,7].map(n=><span className={n<=step?"active":""} key={n}>{n}</span>)}</div><div className="tool-panel">
    {step===1&&<><p className="eyebrow">Schritt 1 von 7</p><h2>Was möchten Sie bewerten?</h2><div className="type-grid">{propertyTypes.map(([type,icon])=><button type="button" className={data.type===type?"selected":""} onClick={()=>update("type",type)} key={type}><span>{icon}</span><strong>{type}</strong></button>)}</div>{data.type==="Haus"&&<><p className="eyebrow">Bauweise *</p><div className="condition-grid">{houseStyles.map(([value,label])=><button type="button" className={data.EGART===value?"selected":""} onClick={()=>update("EGART",value)} key={value}><strong>{label}</strong></button>)}</div></>}</>}
