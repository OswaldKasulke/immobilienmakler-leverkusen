...
const standardStufen=[["1","1 – sehr einfach"],["2","2 – sehr einfach – einfach"],["3","3 – einfach"],["4","4 – einfach – mittel"],["5","5 – mittel"],["6","6 – mittel – gehoben"],["7","7 – gehoben"],["8","8 – gehoben – stark gehoben"],["9","9 – stark gehoben"],["10","10 – Luxus"]];
...
function calculate(data:ValuationData,candidates:AddressCandidate[]):ValuationResult|null{
  ...
  const protocol=[`Adresse: ${data.street} ${data.number}, ${data.zip} ${data.city}${data.district?` · ${data.district}`:""}.`,shared.zone_id?`Richtwertzone: ${shared.zone_id}; Stichtag ${shared.stichtag}; Ausgangswert ${shared.immobilienrichtwert_eur_m2.toLocaleString("de-DE")} €/m².${shared.preisquelle?` (gepflegter Wert statt amtlich ${shared.amtlicher_richtwert_eur_m2?.toLocaleString("de-DE")} €/m²)`:""}`:`Ausgangswert ${shared.immobilienrichtwert_eur_m2.toLocaleString("de-DE")} €/m² für ${shared.lage} (gepflegter Wert, für diese Adresse ist kein amtlicher Richtwert veröffentlicht).`,`Modell: ${shared.modellquelle}; Rechenart ${shared.berechnungsart}.`,`Objektmerkmale: ${JSON.stringify(modelAttributes(data))}.`,`Angepasster Wert: ${shared.pricePerSqm.toLocaleString("de-DE")} €/m²; gerundeter Endwert ${money(shared.value)}; Spanne ±10 % ${money(shared.low)} bis ${money(shared.high)}.`];
...
export default function BewertungsForm(){
  ...
  const [data,setData]=useState<ValuationData>({type:"",EGART:"",street:"",number:"",zip:"",district:"",city:"",living:"",plot:"",rent:"",access:"voll",gstand:"",year:"",first:"",last:"",email:"",phone:"",contactAddress:"",contactZip:"",contactCity:"",consent:"",outside:"",outsideStreet:"",outsideNumber:"",outsideZip:"",outsideCity:""});
  ...
  const canContinue=step===1?!!data.type&&(data.type!=="Haus"||!!data.EGART):step===2?(data.outside==="yes"?!!data.outsideStreet&&!!data.outsideCity:!!data.street&&!!data.zip&&addressValid):step===3?(data.type==="Grundstück"?numeric(data.plot)>0&&!!(data.access||"voll"):!data.gstand?false:data.type==="Mehrfamilienhaus"?numeric(data.living)>0&&numeric(data.rent)>0:numeric(data.living)>0&&(data.type!=="Haus"||numeric(data.plot)>0)):step===4?(istIRW(data.type)?numeric(data.year)>=1800:true):step===5?!!data.first&&!!data.last&&!!data.email&&!!data.phone&&data.consent==="yes":true;
