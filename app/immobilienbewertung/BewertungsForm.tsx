...
const propertyTypes=[["Wohnung","▤","Eigentumswohnung ohne Mietwertverfahren"],["Haus","⌂","Ein- oder Zweifamilienhaus ohne Mietwertverfahren"]];
...
const modelAttributes=(data:ValuationData)=>{const out:Record<string,string|number|boolean>={WHNFL:numeric(data.living),BJ:numeric(data.year)};if(data.type==="Haus"){out.FLAE=numeric(data.plot);out.EGART=numeric(data.EGART);}Object.entries(data).forEach(([key,value])=>{if(/^[A-Z]+$/.test(key)&&value!=="")out[key]=numeric(value);});return out;};
...
export default function BewertungsForm(){
  ...
  const [data,setData]=useState<ValuationData>({type:"",EGART:"",street:"",number:"",zip:"",district:"",city:"",living:"",plot:"",year:"",first:"",last:"",email:"",phone:"",contactAddress:"",contactZip:"",contactCity:"",consent:"",outside:"",outsideStreet:"",outsideNumber:"",outsideZip:"",outsideCity:""});
  ...
  const canContinue=step===1?!!data.type&&(data.type!=="Haus"||!!data.EGART):step===2?(data.outside==="yes"?!!data.outsideStreet&&!!data.outsideCity:!!data.street&&!!data.zip&&addressValid):step===3?numeric(data.living)>0&&(data.type!=="Haus"||numeric(data.plot)>0):step===4?additionalComplete:step===5?numeric(data.year)>=1800:step===6?!!data.first&&!!data.last&&!!data.email&&!!data.phone&&data.consent==="yes":true;
