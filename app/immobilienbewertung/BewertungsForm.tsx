...
function calculate(data:ValuationData,candidates:AddressCandidate[]):ValuationResult|null{
  ...
  if(!istIRW(data.type)){
    ...
    // Jahresnettokaltmiete selbst tauchte aber nirgends auf - der Wert war
    // nicht nachrechenbar.
    const protokollNeu=[adresse,`Verfahren: ${shared.verfahren}; ${shared.berechnungsart}.`,`Grundlage: ${shared.modellquelle}.`,`Rechnung: ${basisNeu} = ${money(shared.value)}.`,`Gerundeter Endwert ${money(shared.value)}; Spanne ±10 % ${money(shared.low)} bis ${money(shared.high)}.`];
...
export default function BewertungsForm(){
  ...
  return <div className="valuation-tool"><div className="progress" aria-label={`Schritt ${step} von 6`}>{[1,2,3,4,5,6].map(n=><span className={n<=step?"active":""} key={n}>{n}</span>)}</div><div className="tool-panel">
    ...
    {step===2&&<><p className="eyebrow">Schritt 2 von 6</p><h2>Adresse der Immobilie</h2><p className="tool-intro">Wo befindet sich die Immobilie?</p><StreetSearch postalCode={data.zip} street={data.street} onPostalCodeChange={(value)=>update("zip",value)} onStreetChange={(value)=>update("street",value)} onResolve={resolved}/></>}{step===3&&<><p className="eyebrow">Schritt 3 von 6</p><h2>Wie groß ist die Immobilie?</h2>{data.type!=="Grundstück"&&<label>{data.type==="Mehrfamilienhaus"?"Wohnfläche in m² (optional)":"Wohnfläche in m² *"}<input inputMode="decimal" value={data.living} onChange={e=>update("living",e.target.value)} placeholder="z. B. 120"/></label>}{(data.type==="Haus"||data.type==="Grundstück")&&<label>{data.type==="Haus"?"Grundstücksfläche in m² (optional)":"Grundstücksfläche in m² *"}<input inputMode="decimal" value={data.plot} onChange={e=>update("plot",e.target.value)} placeholder="z. B. 450"/></label>}{data.type==="Mehrfamilienhaus"&&<label>Monatliche Nettokaltmiete in € *<input inputMode="decimal" value={data.rent||""} onChange={e=>update("rent",e.target.value)} placeholder="z. B. 4200"/></label>}{data.type!=="Grundstück"&&<label>{data.type==="Mehrfamilienhaus"?"Zustand und Ausstattung (optional)":"Zustand und Ausstattung *"}<select value={data.gstand||""} onChange={e=>update("gstand",e.target.value)}><option value="">Bitte auswählen</option>{standardStufen.map(([wert,text])=><option value={wert} key={wert}>{text}</option>)}</select></label>}</>}
